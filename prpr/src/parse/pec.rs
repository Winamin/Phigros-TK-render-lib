...
// 分块加载版本的 parse_judge_line
fn parse_judge_line_chunked(mut pec: PECJudgeLine, id: usize, max_time: f32, r: &mut BpmList, chunked_chart: &mut ChunkedChart) -> Result<JudgeLine> {
    // 将音符分配到不同的chunk
    for note in &pec.notes {
        let chunk_id = chunked_chart.get_chunk_for_time(note.time);
        
        if chunk_id < chunked_chart.chunks.len() {
            if !chunked_chart.chunks[chunk_id].line_ids.contains(&id) {
                chunked_chart.chunks[chunk_id].line_ids.push(id);
            }
        }
    }
    
    // 只加载第一个chunk的音符
    pec.notes.retain(|note| {
        let chunk_id = chunked_chart.get_chunk_for_time(note.time);
        chunk_id == 0
    });
...
pub fn parse_pec_with_list_chunked(source: &str, extra: ChartExtra, _r: &mut BpmList, data_dir: std::path::PathBuf) -> Result<Chart> {
    ...
    let mut lines = lines
        .into_iter()
        .enumerate()
        .map(|(id, line)| parse_judge_line_chunked(line, id, max_time, bpm!(), &mut chunked_chart).with_context(|| ptl!("judge-line-location", "jlid" => id)))
        .collect::<Result<Vec<_>>>()?;
    process_lines(&mut lines);
    ensure_bpm(&mut r, &mut bpm_list);
