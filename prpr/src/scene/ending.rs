...
impl Scene for EndingScene {
    ...
    fn render(&mut self, tm: &mut TimeManager, ui: &mut Ui) -> Result<()> {
        ...
        draw_parallelogram_ex(
            Rect::new(r.x, r.y + r.h * (1. - ratio), r.w - r.h * (1. - ratio) * slope, r.h * ratio),//-1
            None,
            Color::default(),
            Color::new(0., 0., 0., 0.6),
            true,
        );
        let rr = draw_text_aligned(ui, &self.info.level, r.right() - r.h / 7. * 13. * 0.13 - 0.029, r.bottom() - top / 18.5, (1., 1.), 0.40, WHITE);
        ...
        draw_parallelogram(main, None, c2, true);
        {
            ...
            // NEW BEST + SCORE
            let r = draw_text_aligned(ui, &text, main.x + dx + 0.01 - 0.006, main.bottom() - 0.040, (0., 1.), 0.29, Color::new(1., 1., 1., pa));
            let r = draw_text_aligned(ui, &format!("{:07}", res.score), r.x - 0.012 + 0.006, r.y - 0.019, (0., 1.), 1.05, Color::new(1., 1., 1., pa));
        ...
        draw_parallelogram(s2, None, c2, true);
        {
            ...
            draw_count(ui, 0.127, "Perfect", res.counts[0]);
            draw_count(ui, 0.325, "Good", res.counts[1]);
            draw_count(ui, 0.46, "Bad", res.counts[2]);
            draw_count(ui, 0.595, "Miss", res.counts[3]);
